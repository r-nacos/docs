...
title: index
encrypt: false
permalink: /docker_cluster_deploy/
---
# 第一章：简介
...
> [!IMPORTANT]
>
> TPS（Transactions Per Second）和 QPS（Queries Per Second）都是衡量系统性能的指标，但它们关注的方面有所不同。
>
> * **TPS（每秒事务数）**：
...
| 读取     | 每个节点都能读取全量数据                                     | 单节点 30 000 qps 左右。集群总容量为 n*3 万 |
...
| RNACOS_RAFT_NODE_ADDR | 节点地址 IP:GrpcPort                                  | 127.0.0.1:9848 | 单节点运行时每次启动都会生效；<br>多节点集群部署时，只取加入集群时配置的值。 |
...
![image-20240312135638114](/assets/zh/docker_cluster_deploy/1.png)
...
![](/assets/zh/docker_cluster_deploy/2.gif)
...
![](/assets/zh/docker_cluster_deploy/3.gif)
...
![](/assets/zh/docker_cluster_deploy/4.gif)

* 通过浏览器（地址是 `http://192.168.10.100:10848/` ）访问：

![image-20240312140306007](/assets/zh/docker_cluster_deploy/5.png)

## 2.5 不使用 Docker 部署

### 2.5.1 概述

* 部署方式和 [软件包单机部署](./01.单机部署.md) 类似，只不过 r-nacos 支持通过环境变量或配置文件启动，对应参数见 [运行参数说明](../04.运行参数说明.md) 。

### 2.5.2 集群规划

* 使用三台 Linux 主机作为 r-nacos 的节点，即：

![image-20240312140920694](/assets/zh/docker_cluster_deploy/6.png)
