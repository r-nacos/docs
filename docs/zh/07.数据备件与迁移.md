...
r-nacos 运行时使用自定义的分布式文件存储，默认不太方便管理数据。

为了支持用户更方便的管理、使用 r-nacos 中数据，计划通过语言无关的中间数据文件做数据载体，间接支持 r-nacos 系统数据和通用数据库数据双向迁移。
...
1、通过设置备份数据接口 token 参数 `RNACOS_BACKUP_TOKEN` 的值，开启数据备份接口。备份数据 token 默认为空，也就是默认不启用。
