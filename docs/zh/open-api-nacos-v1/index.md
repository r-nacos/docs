...
**接口描述：** 触发本节点进入 Raft「禁写」状态。触发后，该节点将拒绝一切 Raft 写入与通信操作（日志提交、日志复制，以及投票、日志追加、快照安装等请求），等同于将节点安全地从集群写入流程中隔离。适用于节点安全停机、数据备份或迁移前冻结写入，避免数据被继续修改。该接口为**免鉴权**接口，安全性依赖标记文件 `close_raft_mark` 这一物理闸门——只有能访问节点文件系统的人才能触发。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| ok | int | `1` 表示已成功进入禁写状态；`0` 表示未执行（缺少标记文件） |
| msg | string | 仅当 `ok=0` 时返回，提示需先创建标记文件 |
...
- 禁写状态**单向不可逆**，触发后需重启节点才会重新加载，请仅在确需冻结写入时使用。
